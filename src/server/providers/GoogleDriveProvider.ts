...
export class GoogleDriveProvider extends BaseProvider {
  validateUrl(url: string): boolean {
    try {
      ...
      return urlObj.hostname.includes('drive.google.com') || urlObj.hostname.includes('docs.google.com');
    } catch {
      return false;
    }
  }
  ...
  async checkLink(url: string): Promise<CheckResult> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (text.includes('File not found') || text.includes('No longer available') || text.includes('Sorry, you can')) {
          return {
        ...
        if (text.includes('Sign in') || text.includes('requires permission')) {
          return {
  ...
  async extractMetadata(url: string): Promise<Partial<FileMetadata>> {
    try {
      ...
      // Try to extract file size from Google Drive page
